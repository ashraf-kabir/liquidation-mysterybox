...
class Admin_subscription_api_controller extends  Admin_api_controller{
    ...
    public function subscribe_user()
    {
        ...
        if(strlen($user_obj->stripe_id ?? "") < 3)
        {
            return $this->render([
                'code' => 403,
                'success' => FALSE,
                'message' => "payment details on set"
            ], 403);  
            exit();
        }
    ...
    public function index()
    {
     ...
     /**
     * @route v1/api/subscription/user
     */
    public function add_payment_details()
    {
        ...
        if($this->form_validation->run() === FALSE)
        {
            return $this->render(['code' => 403,'success' => FALSE,'message' => strip_tags(validation_errors())], 403);  
            exit();
        }
        ...
        $plan_id = $this->input->post('plan_id') ?? 1;
        $user_obj = $this->user_model->get($user_id);
        $plan_obj = $this->stripe_plans_model->get($plan_id);
        $coupon_obj = NULL;
        ...
        if(empty($plan_obj))
        {
            return $this->render(['code' => 403, 'success' => FALSE, 'message' => "Invalid Plan"], 403);  
            exit();
        }
        ...
        {
            $coupon_obj = $this->stripe_coupons_model->get_by_field('slug',$coupon);
            if(empty($coupon_obj) || (int) $coupon_obj->status === 0  || $coupon_obj->current_usage_limit >= $coupon_obj->usage_limit )
            {
                return $this->render(['code' => 403, 'success' => FALSE, 'message' => "Invalid Coupon"], 403);  
                exit();
            }
        }
        ...
        if(isset($stripe_client['id']))
        {
            ...
            if(!empty($coupon_obj) && (int) $coupon_obj->coupon_type == 0)
            {
                //custom csoupon add free month
                $expiry_date = date("Y-m-d", strtotime("+1 month"));
                $this->user_model->edit(['expire_at' => $expiry_date],$user_id);
                return $this->render(['code' => 200,'success' => TRUE,'message' => "xyzPayment details added"], 200);  
                exit();
            }
            //normal stripe subscription       
            $stripe_subscription_params = [];
           
            $stripe_subscription_params['customer'] = $stripe_client['id'];
            $stripe_subscription_params['items'] = ['plan' => $plan_obj->stripe_id];
            try
            {
                $stripe_subscription = $this->subscribe($stripe_subscription_params, $coupon_obj->id ?? 0, $plan_id , $user_id , 0);
            } 
            catch(Exception $e)
            {
                return $this->render(['code' => 500,'success' => FALSE,'message' => $e->getMessage()], 500);
        ...
        return $this->render(['code' => 403,'success' => FALSE, 'message' => 'xyzSubscription Failed'], 403); 
        exit();
    }

               ...
               <nav class="woocommerce-breadcrumb"><a href="<?php echo base_url(); ?>">Home</a><span><i class="fa fa-angle-right" aria-hidden="true"></i></span>
                    ...
                    <?php if (isset($category->name)) : ?>
                         ...
                         <?php else : ?>
                              ...
                              </i> </span><?php if (isset($_GET['search_term']) and !empty($_GET['search_term'])) { ?> > Search results <?php } ?>
               </nav>
          </div>
     </div>
     ...
     <div class="row">
          <div class="col-md-3">
               <aside id="sidebar-shop" class="widget-area site-sidebar style-2" role="complementary">
                    <section id="tz_categories-2" style="width: 100%;" class="widget widget_tz_categories">
                         <h3 style="margin: 0px;" class="widget-title" itemprop="name"><span>Mystery Box</span></h3>
                                             ...
                                             <?php if (!empty($value->childs_list)) : ?>
                                                  <div class="child-list-cat flex-top-column pl-3">
                                                       <?php foreach ($value->childs_list as $childkey => $childvalue) : ?>
                                                            <a <?php if (isset($_GET['category']) and $_GET['category'] == $childvalue->id) : ?> style="width:100%;" class="active_category" <?php endif; ?> href="<?php echo base_url(); ?>categories/?category=<?php echo $childvalue->id; ?>"><?php echo $childvalue->name; ?></a>
                                                       <?php endforeach ?>
                                                  </div>
                                             <?php endif ?>
                                        </li>
               ...
               <?php if (isset($_GET['search_term']) and !empty($_GET['search_term'])) { ?>
                    <div class="row">
                         <div class="col-sm-12 col-md-12">
                              <main class="site-content store-content" itemscope="itemscope" style="margin-bottom: 1.5rem;" itemprop="mainContentOfPage" role="main">
                                   <!-- Main content -->
                                   <header class="woocommerce-products-header">
                                        <h1 class="woocommerce-products-header__title page-title">Search results for “<?php echo htmlentities($_GET['search_term']); ?>”</h1>
                                   </header>
                         </div>
                    </div>
               <?php } ?>
               ...
               <div class="row flexlist">
                    <?php if (!empty($products_list)) {  ?>
                         <?php foreach ($products_list as $key => $value) { ?>
                              <div class="col-12 col-lg-4 col-md-4 slick-current slick-center flexlist-item">
                                   <div class="offer__item">
                                        <a href="<?php echo base_url(); ?>product/<?php echo $value->id; ?>" tabindex="-1">
                                             ...
                                             <?php } ?>
                                             <!-- <h5 class="text-gray mt-2"><?php if ($value->product_type == 1) {
                                                                                     echo "Regular";

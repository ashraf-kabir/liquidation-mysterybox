...
class Inventory_model extends Manaknight_Model
{
   ...
   * @return array
   */
  public function get_all_inventory_products($search_product_value = false, $where2 = [], $page = 0, $limit = 10)
  {
  ...
  public function get_custom_count($parameters, $parameters2 = [])
  {
    if (!empty($parameters)) {
      ...
      if (isset($parameters['product_name']) && !empty($parameters['product_name'])) {
        $this->db->group_start();
    ...
    if (isset($parameters2) && !empty($parameters2)) {
      $this->db->group_start();
  ...
  public function get_custom_paginated($page = 0, $limit = 10, $where = [], $order_by = '', $direction = 'ASC', $parameters2 = [])
  {
    ...
    if (!empty($where)) {
      foreach ($where as $field => $value) {
        if ($field == 'product_name' || $field == 'sku') {
          continue;
        }
  ...
  public function get_all_inventory_items_for_manifest($query = '', $status = '', $offset = '', $limit = '')
  {
    $this->db->from('inventory');
    $this->db->select('inventory.*, category.name as category_name, physical_location.name as location');
    if ($query != '') {
      $items = $this->db->like('inventory.product_name ', $query);
    }
    ...
    if ($status) {
      $this->db->where('inventory.status', $status);
    }

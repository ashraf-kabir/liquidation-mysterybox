				...
				<div class="form-group col-md-5 col-sm-12">
                    ...
                    <select name="store_id" id="form_store_id" required class="form-control">
                        ...
                        foreach ($stores as $store) {
                            $selected = $this->_data['view_model']->get_store_id() == $store->id? 'selected' : '';
                            echo "<option {$selected} value='{$store->id}'> {$store->name} </option>";
                        } 
                        ?>
                    </select>
					<!-- <input type="text" class="form-control data-input" id="form_store_id" name="store_id" value="<?php echo set_value('store_id', $this->_data['view_model']->get_store_id());?>" onkeypress="return (event.charCode >= 48 && event.charCode <= 57) || (event.charCode == 45)"/> -->
				</div>

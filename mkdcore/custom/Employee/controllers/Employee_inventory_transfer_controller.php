 ...
 *
 */
class Employee_inventory_transfer_controller extends Employee_controller
{
    ...
    public function index($page)
    {
        ...
        include_once __DIR__ . '/../../view_models/Inventory_transfer_employee_list_paginate_view_model.php';
        $session = $this->get_session();
        $format = $this->input->get('format', TRUE) ?? 'view';
        $order_by = $this->input->get('order_by', TRUE) ?? '';
        $direction = $this->input->get('direction', TRUE) ?? 'ASC';

        $this->_data['view_model'] = new Inventory_transfer_employee_list_paginate_view_model(
        ...
        return $this->render('Employee/Inventory_transfer', $this->_data);
    }

    public function add()
    {
        include_once __DIR__ . '/../../view_models/Inventory_transfer_employee_add_view_model.php';
        $session = $this->get_session();
        ...
        $this->_data['view_model'] = new Inventory_transfer_employee_add_view_model($this->inventory_transfer_model);
        $this->_data['view_model']->set_heading('Inventory Transfer Requests');


        if ($this->form_validation->run() === FALSE) {
            return $this->render('Employee/Inventory_transferAdd', $this->_data);
        }
        ...
        if ($result) {
            ...
            return $this->redirect('/employee/inventory_transfer/0', 'refresh');
        }
    ...
    public function view($id)
    {
        ...
        if (!$model) {
            $this->error('Error');
            return redirect('/employee/inventory_transfer/0');
        }


        include_once __DIR__ . '/../../view_models/Inventory_transfer_employee_view_view_model.php';
        $this->_data['view_model'] = new Inventory_transfer_employee_view_view_model($this->inventory_transfer_model);
        $this->_data['view_model']->set_heading('Inventory Transfer Requests');
        ...
        return $this->render('Employee/Inventory_transferView', $this->_data);
    }
    ...
    public function accept($id)
    {
        ...
        if (!$model) {
            $this->error('Error');
            return redirect('/employee/inventory_transfer/0?order_by=id&direction=DESC');
        }

$(document).ready(() => {
      ...
      * 
     */
     function load_tax_value() 
     { 
          $.ajax({
               type: "GET",
               url: "../v1/api/load_tax_value",
               timeout: 15000,
               dataType: "JSON", 
     ...
     //past orders
     function load_pos_past_orders(
     search_past_orders_from = "",
     search_past_orders_to = "",
     search_past_orders_customer = "",
     past_orders_page_no = 1
     ) {
     $.ajax({
          type: "POST",
          url:
          "../v1/api/pos_past_order?past_orders_page_no=" + past_orders_page_no,
          timeout: 15000,
          dataType: "JSON",
          data: {
          start_date: search_past_orders_from,
          end_date: search_past_orders_to,
          customer_name: search_past_orders_customer,
          },
          success: function (response) {
          if (response.pos_past_order) {
               $("#past-order tbody").html(response.pos_past_order);
               $(".pagination-for-past-orders").html(
               response.past_order_paginations
               );
          }
          },
     });
     ...
     $.ajax({
          ...
          if (response.pos_cart_items_list) {
               ...
               cart_list +=
               '<li class="row w-100 align-items-center added-item" data-id="' +
               obj.product_id +
               '" data-toggle="tooltip" data-placement="top" title="Click to Edit Quantity">';

               cart_list +=
               '<span class="col-4"> <h4 class="d-block py-0 my-0">' +
               obj.product_name +
               '</h4> <small class="py-0 my-0">1 Unit(s) at $' +
               Number(obj.unit_price).toFixed(2) +
               "</small></span>";

               cart_list +=
               '<span class="col-3 text-primary">Qty: ' +
               obj.product_qty +
               '</span> <span class="col-3 text-danger text-center">$' +
               Number(obj.total_price).toFixed(2) +
               "</span>";
     ...
     $(".pos-products-list").append(
          '<br><img  class="loader_img_gif" style="width: 140px;object-fit: cover;" src="' +
          loading_gif +
          '"   alt="loading" />'
     );
     $.ajax({
          ...
          timeout: 15000,
          data: {
          search_product_value: search_product_value,
          next_page: next_page,
          },
          dataType: "JSON",
          success: function (response) 
          ...
          if (response.products_list.length == 0) {
               ...
               pos_products +=
                    '<div class="w-100"> <img src="../assets/pos_images/default_product_image.jpg" alt="' +
                    obj.product_name +
                    '" class="mx-auto"> </div>';
               }
     ...
     const displayCartItems = (cartItemsArray) => {
          ...
          taxPrice = ( parseFloat(tax_value) / 100 ) * parseFloat(totalPrice);
          ...
          { 
               ...
               <span class="col-4">
               <h5 class="d-block py-0 my-0"> ${item.name}</h5>
               <small class="py-0 my-0">1 Unit(s) at $${Number(item.price).toFixed(
                    2
               )}</small>
               </span>
               <span class="col-3 text-primary" style="font-size: 13px;">Qty: ${
               item.quantity
               }</span>
               <span class="col-3 text-danger text-center" style="font-size: 13px;">$${(
               Number(item.price) * Number(item.quantity)
               ).toFixed(2)}</span>
               <span class="col-2 text-right" style="font-size: 14px;"><i class="fa fa-trash-alt remove-item" data-id=${
               item.id
               } ></i></span>
               </li>`;
          ...
          for (let i = 0; i < $(".item-tax-value").length; i++) 
          {
               $(".item-tax-value")[i].innerHTML = taxPrice; 
          }
          ...
          for (let i = 0; i < $(".item-subtotal").length; i++) 
          {
               $(".item-subtotal")[i].innerHTML = subTotalPrice; 
          }
     ...
     $.ajax({
          ...
          timeout: 15000,
          data: { form_data: formData, cart_items: cartItems, discount: discount, taxPrice: taxPrice },
          dataType: "JSON",
          success: function (response) {
          if (response.error) {
               toastr.error(response.error);
     ...
     $(".shipping-cost-options").html(
          '<img style="width:60px;object-fit: cover;" src="' +
          loading_gif +
          '"   alt="loading" />'
     );
     ...
     $.ajax({
          ...
          if (response.list_all) {
               ...
               '<label>Service </label><select class="form-control shipping-cost-price" name="shipping_service_id"><option value="">Select Service</option>';
               ...
               shipping_options +=
               '</select> <br> <label>Shipping Cost </label> <input type="hidden" class="form-control shipping_service_name" name="shipping_service_name" value="" /> <input type="text" class="form-control shipping-cost-price-value" name="shipping_cost" value="0" /> ';

               shipping_options +=
               '<p class="expected_total" >Expected Total = <span class="expected_total_value" >0.00</span> </p>';
     ...
     var shipping_service_name = $(this)
          .find(":selected")
     ...
     $(".shipping-cost-price-value").val(
          Number(total_shipping_price).toFixed(2)
     );
     ...
     return fetch(base_url + "v1/api/stripe_terminal_connection_token", {
          method: "POST",
     })
     ...
     fetchPaymentIntentClientSecret(amount).then(function (client_secret) {
          ...
          testCardNumber: card_number_is_stripe,
          });
     ...
     function calculate_expected_total_with_shipping_cost() {
          ...
          var total_after_shipping_cost =
               Number(shipping_cost_price) + Number(totalPrice);
     ...
     $(document).on("click", "#split-payment", function () 
     {
          ...
          {
               $('.is_split_payment').show();
          } else {
               $('.is_split_payment').hide();
               $('#checkout-cash-amount').val('');
               $('#checkout-credit-amount').val('');
          } 
     });

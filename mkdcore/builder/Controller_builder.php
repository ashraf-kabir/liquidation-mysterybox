...
class Controller_builder extends Builder
{
  ...
  protected function output_list_input_add($controller)
  {
    ...
    foreach ($fields as $key => $field) {
      ...
      if (strpos($field[1], 'image') !== FALSE) {
        ...
        $result .= "\t\t\t\t\t<label for=\"{$field[3]}\">{$field[3]} </label>\n";
        $result .= "\t\t\t\t\t<img id=\"output_{$field[0]}\" style=\"max-height:100px\" onerror=\\\"if (this.src != '/uploads/placeholder.jpg') this.src = '/uploads/placeholder.jpg';\\\"/>\n";
  ...
  protected function output_list_input_edit($controller)
  {
    ...
    foreach ($fields as $key => $field) {
      ...
      if (strpos($field[1], 'image') !== FALSE) {
        ...
        $result .= "\t\t\t\t<div class=\"form-group col-md-5 col-sm-12 mb-4\">\n";
        $result .= "\t\t\t\t\t<label for=\"{$field[3]}\">{$field[3]} </label>\n";
        $result .= "\t\t\t\t\t<img class='edit-preview-image d-block' style=\"max-height:100px\" id=\"output_{$field[0]}\" src=\"<?php echo set_value('{$field[0]}', \$this->_data['view_model']->get_{$field[0]}());?>\" onerror=\\\"if (this.src != '/uploads/placeholder.jpg') this.src = '/uploads/placeholder.jpg';\\\"/>\n";
    ...
    foreach ($fields as $key => $field) {
      ...
      if ($has_mapping) {
        ...
        if (strpos($field[1], 'image') !== FALSE) {
          ...
          $result .= "\n\t\t\t\t<div class='row mb-4'>\n\t\t\t\t\t<div class='col'>\n\t\t\t\t\t\t<span class='d-block'>{$field[3]}</span>\n\t\t\t\t\t\t<img class=\"img-fluid d-block mb-3 mt-3 view-image\" style='max-height: 100px;' src=\"<?php echo \$view_model->get_{$field[0]}();?>\" onerror=\\\"if (this.src != '/uploads/placeholder.jpg') this.src = '/uploads/placeholder.jpg';\\\"/>\n\t\t\t\t</div>\n\t\t\t\t</div>\n";
        } else {
          //TODO
  ...
  protected function output_list_rows($fields, $controller, $mappings)
  {
    ...
    if ($has_action) {
      ...
      if ($controller['is_edit']) {
        $result .= "\t\t\t\t\t\t\techo '<a class=\"btn btn-primary btn-sm\" target=\"_self\" href=\"/{$controller['portal']}{$controller['route']}/edit/' . \$data->id . '\">xyzEdit</a>';\n";
      }

      if ($controller['is_view']) {
        $result .= "\t\t\t\t\t\t\techo ' <a class=\"btn btn-warning btn-sm\" target=\"_selfhref=\"/{$controller['portal']}{$controller['route']}/view/' . \$data->id . '\">xyzView</a>';\n";
      }
  ...
  protected function output_list_rows_raw($fields, $controller, $mappings)
  {
    ...
    foreach ($fields as $field) {
      ...
      if ($field_name == 'complex') {
      ...
      } elseif ($this->startsWith($field_type, 'link')) {
        $field_list = explode(':', $field_type);
        ...
        switch ($field_type) {
          ...
          case 'uppercasefirst':
            ...
            break;
          case 'url':
            $result .= "\t\t\t\t\t\t\techo \"<td><a  class='btn-link' target='_self' href='\$data->$field_name'>xyzView</a></td>\";\n";
            break;
          case 'json':
    ...
    if ($has_action) {
      ...
      if ($controller['is_edit']) {
        if ($has_join) {
          $result .= "\t\t\t\t\t\t\techo '<a class=\"btn btn-link  link-underline text-underline btn-sm\" target=\"_self\" href=\"/{$controller['portal']}{$controller['route']}/edit/' . \$data->a_id . '\">xyzEdit</a>';\n";
        } else {
          $result .= "\t\t\t\t\t\t\techo '<a class=\"btn btn-link  link-underline text-underline  btn-sm\" target=\"_self\" href=\"/{$controller['portal']}{$controller['route']}/edit/' . \$data->id . '\">xyzEdit</a>';\n";
        }
      }

      if ($controller['is_view']) {
        if ($has_join) {
          $result .= "\t\t\t\t\t\t\techo ' <a class=\"btn btn-link  link-underline text-underline btn-sm\" target=\"_self\" href=\"/{$controller['portal']}{$controller['route']}/view/' . \$data->a_id . '\">xyzView</a>';\n";
        } else {
          $result .= "\t\t\t\t\t\t\techo ' <a class=\"btn btn-link  link-underline text-underline btn-sm\" target=\"_self\" href=\"/{$controller['portal']}{$controller['route']}/view/' . \$data->id . '\">xyzView</a>';\n";
        }
      }

      if ($controller['is_delete'] || $controller['is_real_delete']) {
        if ($has_join) {
          $result .= "\t\t\t\t\t\t\techo ' <a class=\"btn btn-link  link-underline text-underline text-danger btn-sm\" target=\"_selfhref=\"/{$controller['portal']}{$controller['route']}/delete/' . \$data->a_id . '\">xyzRemove</a>';\n";
        } else {
          $result .= "\t\t\t\t\t\t\techo ' <a class=\"btn btn-link  link-underline text-underline text-danger btn-sm\" target=\"_selfref=\"/{$controller['portal']}{$controller['route']}/delete/' . \$data->id . '\">xyzRemove</a>';\n";
        }
      }
    ...
    foreach ($controller['listing_actions'] as $key => $value) {
      ...
      $result .= "\t\t\t\t\t\t\techo (\$condition) ? \"{$nbsp} <a class='btn btn-link link-underline text-underline btn-sm' target='_self' href='{$route}'>{$label}</a>\" : '';\n";
    }
    $result .= "\t\t\t\t\t\t\techo '</td>';";
    return $result;
  }
  ...
  {
    return "<a class=\"btn btn-primary btn-sm\" target=\"_self\" href=\"/{$controller['portal']}{$controller['route']}/add\"><i class=\"fas fa-plus-circle\"></i></a>";
  }
    ...
    foreach ($controller_model as $key => $field) {
      if ($autocomplete_text == $field[0]) {
        if ($field[1] == 'integer' || $field[1] == 'float') {
          $result .= "'{$field[0]}' => \$text";
        } elseif ($field[1] == 'date' || $field[1] == 'datetime') {
          $result .= "'{$field[0]}' => '\$text'";
        } else {
